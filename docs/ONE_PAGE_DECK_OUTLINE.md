...
- **"What's your sample size?"** 66 captured tool sessions: 60 live CLI runs (two vendors × three tasks × ten runs each) plus 6 controlled IDE sessions (two IDE-bound vendors × three tasks, captured once and replayed for CSV-shape consistency), plus 3 human sessions — 600 metric rows. All four testable metrics differ significantly (Kruskal–Wallis, α = 0.01); given the replay design we treat the tests as supporting evidence — the structural findings rest on absolute gaps and direct code inspection.
